...
class word_embedding(nn.Module):
    def __init__(self, vocab_length, embedding_dim):
        ...
        self.word_embedding.weight.data.copy_(
            torch.from_numpy(w_embeding_random_intial))  # 好像是加载已经训练好的词向量
    ...
    def forward(self, input_sentence):
        ...
        """
        sen_embed = self.word_embedding(
            input_sentence)  # 把input_sentence变成embedding吧
        return sen_embed
...
class RNN_model(nn.Module):
    def __init__(self, batch_sz, vocab_len, word_embedding, embedding_dim, lstm_hidden_dim):
        ...
        self.rnn_lstm = nn.LSTM(input_size=self.word_embedding_dim,
                                hidden_size=self.lstm_dim, num_layers=2, batch_first=True)

        ##########################################
        # 这里是一个全连接层，输入大小是上面lstm出来的output=hidden_dim,输出大小是vocab_len
        self.fc = nn.Linear(lstm_hidden_dim, vocab_len,
                            self.word_embedding_dim)
        # call the weights initial function.#全连接层参数初始化去啦
    ...
    def forward(self, sentence, is_test=False):
        batch_input = self.word_embedding_lookup(sentence).view(
            1, -1, self.word_embedding_dim)  # 加载的词向量
        ...
        # 将h、c都初始化为零，进入模型得到输出
        output, _ = self.rnn_lstm(batch_input, (torch.zeros(2, 1, self.lstm_dim).to(
            device), torch.zeros(2, 1, self.lstm_dim).to(device)))
        ################################################
        # 使用contiguous()针对x进行变化，感觉上就是深拷贝。当调用contiguous()时，会强制拷贝一份tensor，让它的布局和从头创建的一模一样，但是两个tensor完全没有联系。改变x之后，对y没有影响
